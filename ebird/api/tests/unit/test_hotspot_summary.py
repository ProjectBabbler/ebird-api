...
from ebird.api.product import HOTSPOT_SUMMARY_URL, hotspot_summary
...
class HotspotSummaryTests(TestCase):
    ...
    @mock.patch('ebird.api.product.get_content', side_effect=get_content)
    def test_url(self, mocked_function):
        """Verify the correct URL is used to fetch the records."""
        hotspot_summary('L12345')
        actual = mocked_function.call_args[0][0]
        self.assertEqual(HOTSPOT_SUMMARY_URL, actual)
