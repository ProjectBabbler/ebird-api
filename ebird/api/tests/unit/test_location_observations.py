...
from ebird.api.data import LOCATION_OBSERVATIONS_URL, location_observations
...
class LocationObservationsTests(TestCase):
    ...
    def test_url(self, mocked_function):
        ...
        location_observations('L12345')
        actual = mocked_function.call_args[0][0]
        self.assertEqual(LOCATION_OBSERVATIONS_URL, actual)
    ...
    def test_parameters(self, mocked_function):
        ...
        for idx, ((args, kwargs), expected) in enumerate(self.parameters):
            location_observations(*args, **kwargs)
