...
from ebird.api.data import GEO_SPECIES_URL, geo_species
...
class GeoSpeciesTests(TestCase):
    ...
    parameters = [
        ...
        (  # Default dist is filtered out
            {'species': 'Apus apus', 'lat': 1.0, 'lng': 0.0, 'dist': 25},
            {'sci': 'Apus apus', 'lat': '1.00', 'lng': '0.00', 'fmt': 'json'}),
        (  # Value for dist is included
            {'species': 'Apus apus', 'lat': 1.0, 'lng': 0.0, 'dist': 10},
            {'sci': 'Apus apus', 'lat': '1.00', 'lng': '0.00', 'dist': 10, 'fmt': 'json'}),
    ...
    validation = [
        ...
        {'species': 'Apus apus', 'lat': 0.0, 'lng': 0.0, 'dist': 51},  # dist is more than 50
        {'species': 'Apus apus', 'lat': 0.0, 'lng': 0.0, 'dist': -1},  # dist is less than 0
        {'species': 'Apus apus', 'lat': 0.0, 'lng': 0.0, 'dist': 'str'},  # dist cannot be converted to an integer
