...
def clean_region_type(value):
    ...
    if cleaned not in REGION_TYPES:
        raise ValueError(
            "Region type, %s, must be one or more of : %s" % (
                value, ', '.join(["'%s'" % item for item in REGION_TYPES])))
    return cleaned
...
def clean_category(value):
    ...
    for entry in cleaned:
        if entry not in SPECIES_CATEGORIES:
            raise ValueError(
                "Species category, %s, must be one or more of : %s" % (
                    entry, ', '.join(["'%s'" % item for item in SPECIES_CATEGORIES])))
...
def clean_ordering(value):
    cleaned = value.lower().strip()
    if cleaned not in SPECIES_ORDERING:
        raise ValueError(
            "Species ordering, %s, must be one or more of : %s" % (
                value, ', '.join(["'%s'" % item for item in SPECIES_ORDERING])))
    return cleaned


def clean_sort(value):
    cleaned = value.lower().strip()
    if cleaned not in SPECIES_SORT:
        raise ValueError(
            "Species sort, %s, must be one or more of : %s" % (
                value, ', '.join(["'%s'" % item for item in SPECIES_SORT])))
    return cleaned
